...
import br.com.tim.model.util.BPLUtility;
import br.com.tim.utils.CommonsConstants;
import br.com.tim.utils.EuroLeagueUtils;

public class BPLMapper<T> extends Mapper<T, Text, EuroLeaguesKey, EuroLeagueslValue> {
	private static Logger LOG = Logger.getLogger(BPLMapper.class);
	...
	public void map(T key, Text value, Context context)  throws IOException, InterruptedException {
		...
		try{
			...
			BPL bpl = BPLUtility.parseToObject(value.toString().split(CommonsConstants.FILE_SPLIT_REGEX, -1));
			
			if (null == bpl) throw new EuroLeagueException("ERROR: BPL game object is null.");
			
			//Verificando se clube ou goal são null
			if(bpl.homeTeam == null || bpl.fullTimeHomeTeamGoals == null || bpl.awayTeam == null || bpl.fullTimeAwayTeamGoals == null) return;
			if(bpl.homeTeam .isEmpty() || bpl.fullTimeHomeTeamGoals .isEmpty() || bpl.awayTeam.isEmpty() || bpl.fullTimeAwayTeamGoals.isEmpty()) return;
			
			try{
				outKey.set(EuroLeagueUtils.parseData(bpl.matchDate));
				outValue.set(bpl);
